---
title: Partenaires
---

Le PG Day France ne serait pas possible sans le soutien des entreprises de
la communauté PostgreSQL. Nous remercions chaleureusement nos partenaires
pour leur aide et leur présence à nos cotés.

The PG Day France would not be possible without the support of companies from
the PostgreSQL community. We sincerely thank our sponsors for their support and
their presence alongside us.


<div class="row">
  <div class="col-lg-12 text-center">
    <h3 style="margin-top: 40px; margin-bottom: 30px; color: #d4af37;">Gold Sponsors</h3>
  </div>
</div>

<div class="row display-flex" style="display: flex; flex-wrap: wrap; align-items: center; justify-content: center;">
  <div class="col-md-4 col-sm-12">
     <div style="text-align: center;">
       <a href="https://aws.amazon.com/" target="_blank">
         <img src="/img/aws.png" alt="AWS" class="img-responsive" style="max-height: 150px; padding: 30px; margin: 0 auto;" />
       </a>
     </div>
  </div>
  <div class="col-md-4 col-sm-12">
     <div style="text-align: center;">
       <a href="https://www.dalibo.com/" target="_blank">
         <img src="/img/dalibo.png" alt="Dalibo" class="img-responsive" style="max-height: 150px; margin: 0 auto;" />
       </a>
     </div>
  </div>
  <div class="col-md-4 col-sm-12">
     <div style="text-align: center;">
       <a href="https://loxodata.com/" target="_blank">
         <img src="/img/loxodata.png" alt="Loxodata" class="img-responsive" style="max-height: 150px; margin: 0 auto;" />
       </a>
     </div>
  </div>
</div>

<div class="row display-flex" style="display: flex; flex-wrap: wrap; align-items: center; justify-content: center;">
  <div class="col-md-4 col-sm-12">
     <div style="text-align: center;">
       <a href="https://www.navicat.com/" target="_blank">
         <img src="/img/navicat.png" alt="Navicat" class="img-responsive" style="max-height: 150px; margin: 0 auto;" />
       </a>
     </div>
  </div>
</div>

<div class="row">
  <div class="col-lg-12 text-center">
    <h3 style="margin-top: 40px; margin-bottom: 30px; color: #336791;">Community Sponsor</h3>
  </div>
</div>

<div class="row display-flex" style="display: flex; flex-wrap: wrap; align-items: center; justify-content: center;">
  <div class="col-md-12 col-sm-12">
     <div style="text-align: center;">
       <a href="https://aws.amazon.com/" target="_blank">
         <img src="/img/aws.png" alt="AWS" class="img-responsive" style="max-height: 150px; padding: 30px; margin: 0 auto;" />
       </a>
     </div>
  </div>
</div>

<div class="row">
  <div class="col-lg-12 text-center">
    <h3 style="margin-top: 40px; margin-bottom: 30px; color: #C0C0C0;">Silver Sponsors</h3>
  </div>
</div>

<div class="row display-flex" style="display: flex; flex-wrap: wrap; align-items: center; justify-content: center;">
  <div class="col-md-6 col-sm-12">
     <div style="text-align: right;">
       <a href="https://www.leboncoin.fr/" target="_blank">
         <img src="/img/leboncoin.png" alt="LeBonCoin" class="img-responsive" style="max-height: 150px; margin: 0 auto;" />
       </a>
     </div>
  </div>
  <div class="col-md-6 col-sm-12">
     <div style="text-align: left;">
       <a href="https://www.maif.fr/" target="_blank">
         <img src="/img/Logo_Maif_2019.svg.png" alt="MAIF" class="img-responsive" style="max-height: 150px; margin: 0 auto;" />
       </a>
     </div>
  </div>
</div>

<div class="row">
  <div class="col-lg-12 text-center">
    <h3 style="margin-top: 40px; margin-bottom: 30px; color: #cd7f32;">Bronze Sponsors</h3>
  </div>
</div>

<div class="row display-flex" style="display: flex; flex-wrap: wrap; align-items: center; justify-content: center;">
  <div class="col-md-12 col-sm-12">
     <div style="text-align: center;">
       <a href="https://www.data-bene.io/" target="_blank">
         <img src="/img/databene.png" alt="Data Bene" class="img-responsive" style="max-height: 60px; margin: 0 auto;" />
       </a>
     </div>
  </div>
</div>

<div class="row">
  <div class="col-lg-12 text-center">
    <h3 style="margin-top: 40px; margin-bottom: 30px; color: #336791;">Special Thanks</h3>
  </div>
</div>

<div class="row display-flex" style="display: flex; flex-wrap: wrap; align-items: center; justify-content: center;">
  <div class="col-md-12 col-sm-12">
     <div style="text-align: center;">
       <a href="https://meteofrance.com/" target="_blank">
         <img src="/img/MeteoFrance.png" alt="Meteo France" class="img-responsive" style="max-height: 100px; margin: 0 auto;" />
       </a>
     </div>
  </div>
</div>

<hr style="margin: 50px 0;">

## (FR) Devenez partenaire du PG Day France

<div class="table-responsive">
<table class="table table-striped table-bordered">
  <thead>
    <tr class="info">
      <th>Avantages</th>
      <th class="text-center">BRONZE</th>
      <th class="text-center">SILVER</th>
      <th class="text-center">COMMUNITY SPONSOR</th>
      <th class="text-center">GOLD</th>
    </tr>
  </thead>
  <tbody>
  <tr>
    <th>Logo sur le site web</th>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
  </tr>
  <tr>
    <th>Mise à l'honneur lors des sessions d'ouverture et de clôture</th>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
  </tr>
  <tr>
    <th>Invitations VIP</th>
    <td class="text-center">1</td>
    <td class="text-center">2</td>
    <td class="text-center">2</td>
    <td class="text-center">5</td>
  </tr>
  <tr>
    <th>Priorité sur les lightning talks (1 max)</th>
    <td class="text-center"></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
  </tr>
  <tr>
    <th>Annonce et affichage lors de la soirée communautaire</th>
    <td class="text-center"></td>
    <td class="text-center"></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"></td>
  </tr>
  <tr>
    <th>Stand lors des deux jours</th>
    <td class="text-center"></td>
    <td class="text-center"></td>
    <td class="text-center"></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
  </tr>
  <tr>
      <th>Tarif</th>
      <td class="text-center"><strong>500€</strong></td>
      <td class="text-center"><strong>1500€</strong></td>
      <td class="text-center"><strong>2000€</strong></td>
      <td class="text-center"><strong>3000€</strong></td>
    </tr>
  </tbody>
</table>
</div>

<div class="well">
  <h4>Disponibilité :</h4>
  <ul class="list-inline">
    <li><span class="label label-success">Bronze : Illimité</span></li>
    <li><span class="label label-success">Silver : Illimité</span></li>
    <li><span class="label label-danger">Community Sponsor : Épuisé</span></li>
    <li><span class="label label-warning">Gold : 1 ticket disponible</span></li>
  </ul>
</div>

Pour toute demande, veuillez nous contacter à l'adresse **contact@pgday.fr**.

## (EN) Become a sponsor of PG Day France

<div class="table-responsive">
<table class="table table-striped table-bordered">
  <thead>
    <tr class="info">
      <th>Benefits</th>
      <th class="text-center">BRONZE</th>
      <th class="text-center">SILVER</th>
      <th class="text-center">COMMUNITY SPONSOR</th>
      <th class="text-center">GOLD</th>
    </tr>
  </thead>
  <tbody>
  <tr>
    <th>Logo on the website</th>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
  </tr>
  <tr>
    <th>Highlighted during opening and closing sessions</th>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
  </tr>
  <tr>
    <th>VIP Invitations</th>
    <td class="text-center">1</td>
    <td class="text-center">2</td>
    <td class="text-center">2</td>
    <td class="text-center">4</td>
  </tr>
  <tr>
    <th>Priority on lightning talks (1 max)</th>
    <td class="text-center"></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
  </tr>
  <tr>
    <th>Announcement and display during the community evening</th>
    <td class="text-center"></td>
    <td class="text-center"></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
    <td class="text-center"></td>
  </tr>
  <tr>
    <th>Booth during both days</th>
    <td class="text-center"></td>
    <td class="text-center"></td>
    <td class="text-center"></td>
    <td class="text-center"><i class="fa fa-check text-success"></i></td>
  </tr>
  <tr>
      <th>Price</th>
      <td class="text-center"><strong>500€</strong></td>
      <td class="text-center"><strong>1500€</strong></td>
      <td class="text-center"><strong>2000€</strong></td>
      <td class="text-center"><strong>3000€</strong></td>
    </tr>
  </tbody>
</table>
</div>

<div class="well">
  <h4>Availability:</h4>
  <ul class="list-inline">
    <li><span class="label label-success">Bronze: Unlimited</span></li>
    <li><span class="label label-success">Silver: Unlimited</span></li>
    <li><span class="label label-danger">Community Sponsor: Sold Out</span></li>
    <li><span class="label label-warning">Gold: 1 ticket available</span></li>
  </ul>
</div>

For any inquiries, please contact us at **contact@pgday.fr**.


<!--
## Liens / Links

* [Contract (EN)](...)
-->
